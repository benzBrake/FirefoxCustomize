...
ℹ**`addMenuPlus.uc.mjs`依赖`000-$.sys.mjs`和`000-syncify.sys.mjs`**(这两个脚本要和`addMenuPlus.uc.mjs`放在同一个目录)。

⚡**如果使用 alice0775 的原版 userChrome.js loader（不支持 `.uc.mjs` 文件），需要额外使用 `addMenuPlus.loader.uc.js` 适配器**。该适配器会自动注册 AddMenu actor 并在 window 作用域导入 addMenuPlus.uc.mjs，确保功能完整可用。
...
- **推荐方式**：使用支持 `.uc.mjs` 的 loader（如 `000-0loader.sys.mjs`），直接将 `addMenuPlus.uc.mjs` 及其依赖（`000-$.sys.mjs` 和 `000-syncify.sys.mjs`）放在 chrome 目录下即可
- **兼容方式**：如果使用 alice0775 原版 userChrome.js loader（不支持 `.uc.mjs`），需要额外使用 `addMenuPlus.loader.uc.js` 适配器：
  1. 将 `addMenuPlus.uc.mjs` 及其依赖放在 chrome 目录下
  2. 将 `addMenuPlus.loader.uc.js` 放在 chrome 目录下（会自动被 loader 加载）
  3. 重启 Firefox 生效
...
- [\_addmenu 示例合集.js](https://github.com/ywzhaiqi/userChromeJS/blob/master/addmenuPlus/_addmenu%E7%A4%BA%E4%BE%8B%E5%90%88%E9%9B%86.js)
...
tab([
  ...
  },
  {
    ...
    oncommand: function () {
      ...
      ); // 没记错是 FF 78 开始必须提供 triggeringPrincipal 参数
    },
  ...
  },
  {
    ...
    oncommand: function () {
      ...
      var tabs = gBrowser.mTabContainer
        ...
        : gBrowser.tabs; // Firefox 61 删除了 mTabContainer
...
function copyBBS_or_MD(event) {
  var title =
      addMenu.convertText("%RLINK_TEXT%") || addMenu.convertText("%TITLE%"),
    url = addMenu.convertText("%RLINK%") || addMenu.convertText("%URL%");

  [
    " - 互助分享 - 大气谦和!",
    "_免费高速下载|百度云 网盘-分享无限制",
    " - Powered by Discuz!",
    "百度云 网盘-",
    "的分享",
  ].forEach(function (r) {
  ...
  var undoMenu = ToolMenu({
    ...
    onclick: function (event) {
      if (event.button == 2) {
        ...
        event.target.ownerGlobal.PlacesCommandHook.showPlacesOrganizer(
          "History"
        );
      }
    ...
    onpopupshowing: function (e) {
      ...
      var popup = e.target;
      popup
        .querySelector("#addMenu-undo-open-all")
        .setAttribute(
          "label",
          e.target.ownerGlobal.gNavigatorBundle.getString(
            "menuOpenAllInTabs.label"
          )
        );
      popup
        .querySelectorAll(".undo-item")
      ...
      for (let i = 0; i < tabLength; i++) {
        ...
        const m = $C(e.view.document, "menuitem", {
          label: item.title,
          class:
            "undo-item menuitem-iconic bookmark-item menuitem-with-favicon",
          value: i,
          oncommand:
            'event.stopPropagation();event.preventDefault(); undoCloseTab(event.originalTarget.getAttribute("value"));',
        });
...
_注：github.com 由于服务器限制，无法直接插入 js，故无效。_ 现在建议使用扩展翻译：[翻译网页 – 下载 🦊 Firefox 扩展（zh-CN）](https://addons.mozilla.org/zh-CN/firefox/addon/traduzir-paginas-web/)
...
page({
  ...
  image:
    "data:image/png;base64,",
});
```
...
imagesub([
  ...
  },
  {
    ...
    where: "tab",
    image:
      "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAVElEQVQ4ja2SwQ4AMARD+/8/bbdlw8ogcdK9RQsApNmQal2A359dgCK7j07tBmiBByGzWMjABsBMfaaQgdAYxwGtFVomMiGZDRxSYFLulCtFY8z2AmZhBhe3B+XrAAAAAElFTkSuQmCC",
  },
]);
```
...
page({
  ...
  onshowing: function (menuitem) {
    ...
    if (sel && sel.length > 15) sel = sel.substr(0, 15) + "...";
    this.label = "复制: " + sel;
  },
});
```
...
示例：动态读取内置搜索引擎，来自[Firefox 添加右键搜索选中文本（自动读取内置搜索引擎）](https://blog.iplayloli.com/firefox-adds-rightclick-search-to-select-text-automatically-read-the-builtin-search-engine.html)
...
new (function () {
  var items = [
    ...
    {
      label: Services.locale.appLocaleAsBCP47.includes("zh-")
        ? "生成二维码"
        : "Generate QR code",
      where: "tab",
  ...
  var menu = PageMenu({
    ...
    onshowing: function (e) {
      var sel = addMenu.convertText(
        Services.locale.appLocaleAsBCP47.includes("zh-")
          ? "搜索: %SEL%"
          : "Search %SEL% by"
      );
      if (sel && sel.length > 15) sel = sel.substr(0, 15) + "...";
      ...
      popupNode.querySelectorAll(".auto-generated").forEach((m) => {
  ...
  menu(items);
  css(
    "#context-searchselect { display: none } #contentAreaContextMenu #addMenu-search-select .menu-accel-container { visibility: hidden; }"
  );
})();
```
...
示例：输入框右键增加菜单，在光标处插入自定义字符。_这种方式在百度贴吧无效，因其输入框较为特殊，可采用下面通用的复制粘贴的方式。_
...
page({
  ...
  oncommand: function (event) {
    ...
    // 在光标处插入字符
    input.value =
      input.value.slice(0, aStart) + aText + input.value.slice(aEnd);
...
new (function () {
  var items = [
    ...
    },
    {
      ...
      image:
        "data:image/png;base64,",
    },
    {
      label: Services.locale.appLocaleAsBCP47.includes("zh-")
        ? "TinEye搜图"
...
new (function () {
  var saveMedia = function (event) {
    ...
    const Downloads =
      ...
      Cu.import("resource://gre/modules/Downloads.jsm").Downloads;
    let aUrl =
      addMenu.convertText("%MEDIA_URL%") ||
      addMenu.convertText("%LINK_OR_URL%");
    ...
    if (aUrl) {
      ...
      var fileSaving = Cc["@mozilla.org/file/local;1"].createInstance(
        Ci.nsIFile
      );

      // Bug 1565216 to evaluate passing referrer as item.baseURL
      let referrerInfo = new ReferrerInfo(Ci.nsIReferrerInfo.EMPTY, true, aUri);
      ...
      try {
        ...
        if (!fileSaving.isDirectory()) {
          addMenu.alert(
            "The download folder [%s] does not exist!".replace("%s", savePath)
          );
          return;
        }
      } catch (e) {
      ...
      if (SAVE_DOMAIN_NAME) {
        ...
        if (!fileSaving.exists() || !fileSaving.isDirectory()) {
          try {
            ...
            addMenu.alert(
              "Cannot create download path [%s]".replace("%s", fileSaving.path)
            );
          }
        }
      ...
      if (!event.shiftKey && mimeInfo) {
        ...
        while (fileSaving.exists()) {
          if (newFileName.indexOf(".") != -1) {
            ...
            var file = newFileName.substring(
              0,
              newFileName.length - ext.length
            );
        ...
        var urifix = Cc["@mozilla.org/docshell/uri-fixup;1"].getService(
          Ci.nsIURIFixup
        );
        ...
        , addMenu.sandbox)(
          ...
          false,
          null,
          PrivateBrowsingUtils.isBrowserPrivate(gContextMenu.browser),
  ...
  css(
    "#context-saveimage, #context-savevideo, #context-saveaudio, #context-savepage, #context-savelink { display: none !important }"
  );
})();
```
...
mod({
  ...
  onclick: function (e) {
    if (e.button == 1) {
      ...
      const PREF_BRANCH = "userChrome.navbar.youGetBtn.",
        PREF_COOKIES = "COOKIESPATH",
        PREF_BIN = "BINPATH",
        PREF_SAVE = "SAVEPATH";
      ...
      if (!aCookiesDir.exists())
        aCookiesDir.create(Ci.nsIFile.DIRECTORY_TYPE, 0o666);

      if (!aCookiesDir.isReadable() || !aCookiesDir.isWritable()) {
        alert(
          Services.locale.appLocaleAsBCP47.includes("zh-")
            ? "Cookies 保存目录不可读写"
            : "Cookies storage directory cannot be read or written"
        );
        return;
      }
      ...
      if (uri.spec.startsWith("http")) {
        ...
        function setYouGetPath() {
          alert(
            Services.locale.appLocaleAsBCP47.includes("zh-")
              ? "请先设置 you-get.exe 的路径!!!"
              : "Please set you-get.exe path first!!!"
          );
          if (Services.locale.appLocaleAsBCP47.includes("zh-")) {
            openUrl(
              { target: this },
              "https://lussac.lanzoui.com/b00nc5aab",
              "tab"
            );
          } else {
            openUrl(
          ...
          choosePathAndSave(
            Services.locale.appLocaleAsBCP47.includes("zh-")
              ? "设置 you-get.exe 路径"
              : "Set you-get.exe path",
            PREF_BRANCH + PREF_BIN,
            Ci.nsIFilePicker.modeOpen,
            {
              title: Services.locale.appLocaleAsBCP47.includes("zh-")
                ? "执行文件"
                : "Executable file",
              param: "*.exe",
            }
        ...
        function formatCookie(co) {
          return [
            [co.isHttpOnly ? "#HttpOnly_" : "", co.host].join(""),
        ...
        function saveCookie(host) {
          ...
          const converterStream = Components.classes[
