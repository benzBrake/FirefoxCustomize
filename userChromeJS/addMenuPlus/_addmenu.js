...
const LOCALE = typeof Services.locale.getAppLocalesAsBCP47 === "function" ? Services.locale.getAppLocalesAsBCP47() : Services.locale.appLocaleAsBCP47;
app([{}, {
...
ident([
    {
        ...
        label: LOCALE.includes("zh-") ? "网页快照查询" : "Search in 2tool.top",
        url: "https://2tool.top/kuaizhao.php?k=%URL%",
...
new function () {
    var CatGroup = GroupMenu({
        parent: 'identity-icon-box-contextmenu',
        class: 'showFirstText',
    });
    CatGroup([{
...
//         class: 'showText',
//         label: LOCALE.includes("zh-") ? '新建标签页...' : 'New Tab Options...',
//     });
//     groupMenu([{
//         label: "新建标签页",
...
//     }, {
//         label: LOCALE.includes("zh-") ? "复制标签页" : "Clone Tab",
...
new function () {
    var groupMenu = new GroupMenu({
        ...
        label: LOCALE.includes("zh-") ? '移动标签页...' : 'Move Tab Options...',
    });
    groupMenu([{
        label: "移动到开头",
        class: "fixedSize",
...
new function () {
    var groupMenu = new GroupMenu({
        id: 'addMenu-tab-copylink-1',
        class: 'showText',
        insertBefore: 'context_reopenInContainer',
        label: LOCALE.includes("zh-") ? '标题/地址操作...' : 'Title & Link operate...',
        onshowing: function () {
...
new function () {
    var groupMenu = new GroupMenu({
        ...
        class: 'showFirstText',
        insertBefore: 'context_reopenInContainer',
        label: LOCALE.includes("zh-") ? 'Favicon 操作...' : 'Favicon operate...',
        onshowing: function () {
...
new function () {
    ...
    css('#context-viewsource, #context-inspect-a11y, #context-inspect { display: none }');
};
// 站内搜索
new function () {
...
new function () {
    var groupMenu = GroupMenu({
        ...
        condition: 'link',
        insertAfter: 'context-openlinkincurrent',
        onshowing: function () {
    ...
    groupMenu([{
        ...
        label: LOCALE.includes("zh-") ? "在新标签中打开" : "Open in New Tab",
        oncommand: function (event) {
        ...
        label: LOCALE.includes("zh-") ? "在无痕标签中打开" : "Open in Private Tab",
    ...
    css("#openLinkInPrivateTab,#context-openlinkintab,#context-openlinkincontainertab,#context-openlinkincurrent { display:none }");
};
...
new function () {
    ...
    groupMenu([{
        label: LOCALE.includes("zh-") ? '新窗口打开' : 'Open in new window',
        ...
        label: LOCALE.includes("zh-") ? '在隐私窗口中打开' : 'Open in private window',
...
new function () {
    var groupMenu = new GroupMenu({
        class: 'showFirstText',
        condition: 'select',
        insertBefore: 'context-print-selection',
        onshowing: function (target) {
            this.lastChild.setAttribute("tooltiptext", document.getElementById("context-print-selection").label);
        },
    });
    groupMenu([{
        label: LOCALE.includes("zh-") ? '快速保存选定文本' : 'Save slected text quickly',
    ...
    css("#context-print-selection { visibility: collapse }");
}
// 搜索文本
new function () {
    var items = [{
