...
new function () {
    var groupMenu = new TabGroup({
        ...
        onshowing: function (item) {
            item.hidden = item.ownerGlobal.gBrowser.currentURI.spec.startsWith('file')
        }
    });
    groupMenu([{
...
new function () {
    var groupMenu = new TabGroup({
        ...
        onshowing: function (item) {
            const { gBrowser } = item.ownerGlobal;
...
new function () {
    var groupMenuNormal = new PageGroup({
        ...
        condition: 'normal',
        onshowing: function (item) {
            item.hidden = !item.ownerGlobal.gBrowser.currentURI.spec.startsWith('http');
        }
    }),
    ...
    var items = [{
        ...
        onshowing: function (item) {
            item.hidden = !item.ownerGlobal.gBrowser.currentURI.host.includes("www.baidu.com");
        },
        oncommand: function (e) {
            const locale = addMenu.locale;
            var sel = (gContextMenu || { textSelected: "" }).textSelected;
...
new function () {
    var groupMenu = GroupMenu({
        ...
        onshowing: function (item) {
            ...
            let privateBtn = win.document.getElementById('addMenu-openLink-tab-private')
            if (privateBtn) {
                if (typeof win.UC === "undefined") {
                    privateBtn.hidden = true;
                    return;
                }
                privateBtn.hidden = typeof win.UC.privateTab === "undefined";
            }
        }
    });
    groupMenu([{
...
page([{
    ...
    text: "%SVG_BASE64%",
    onshowing: function (item) {
        let uri = item.ownerGlobal.gBrowser.currentURI.spec;
        if (!uri.endsWith(".svg")) {
            item.hidden = true;
        } else {
            item.hidden = false;
...
new function () {
    var groupMenu = new GroupMenu({
        ...
        onshowing: function (item) {
            const { document } = item.ownerGlobal;
...
new function () {
    ...
    var menu = PageMenu({
        ...
        onshowing: function (item) {
            const { Services, document, addMenu } = item.ownerGlobal;
