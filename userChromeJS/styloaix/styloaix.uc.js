...
(function () {
  UC.styloaix = {
    ...
    createButton (doc) {
      let btn = _uc.createElement(doc, 'toolbarbutton', {
        ...
        onclick: function (event) {
          if (event.button === 1 && event.target.id == this.id) {
            UC.styloaix.enabled = !UC.styloaix.enabled;
          }
        }
      });
      ...
      let reloadAllBtn = _uc.createElement(doc, 'menuitem', {
        ...
        oncommand: function () {
          UC.styloaix.toggleAll({reload: true});
        }
      });
      ...
      let newPageStyleBtn = _uc.createElement(doc, 'menuitem', {
        label: 'For this page',
        oncommand: function () {
          let uri = UC.styloaix.getCurrentURI();
          uri
            ? UC.styloaix.openEditor({url: uri.specIgnoringRef, type: "url"})
            : UC.styloaix.openEditor();
        }
      });
      ...
      let styleEdit = _uc.createElement(doc, 'menuitem', {
        ...
        label: 'Edit',
        oncommand: function () {
          UC.styloaix.openEditor({id: this.parentNode.triggerNode._style.fullName});
        }
      });
      ...
      let styleReload = _uc.createElement(doc, 'menuitem', {
        ...
        label: 'Reload',
        oncommand: function () {
          this.parentNode.triggerNode._style.reload();
        }
      });
      ...
      let styleDelete = _uc.createElement(doc, 'menuitem', {
        ...
        label: 'Delete',
        oncommand: function () {
          this.parentNode.triggerNode._style.delete();
        }
      });
    ...
    addStyleInMenu (style, popup) {
      let menuitem = _uc.createElement(popup.ownerDocument, 'menuitem', {
        ...
        onmouseup: function (event) {
          UC.styloaix.shouldPreventHide(event, this._style);
        },
    ...
    get RESOURCE_URI () {
      // xiaoxiaoflood maps assets under chrome://userchromejs/content/,
      // while this fake loader exposes them through chrome://userchromejs/content/utils/.
      return _uc.isFaked === true
        ? 'chrome://userchromejs/content/utils/styloaix/'
        : 'chrome://userchromejs/content/styloaix/';
    },
    get EDITOR_URI () {
      return this.RESOURCE_URI + 'edit.xhtml';
    },
        ...
        return this.url = Services.io.newURI('data:text/css;charset=UTF-8,' + encodeURIComponent(`
          @-moz-document url('${_uc.BROWSERCHROME}'), url('chrome://messenger/content/customizeToolbar.xhtml'), url('${UC.styloaix.TBDETACHEDMAIL}'), url('${UC.styloaix.TBMAILCOMPOSEWIN}') {
            #styloaix-button.icon-white {
              list-style-image: url('${UC.styloaix.RESOURCE_URI}16w.png');
            }
            #styloaix-button.icon-colored {
              list-style-image: url('${UC.styloaix.RESOURCE_URI}16.png');
            }
            ...
            #styloaix-popup menuitem[type="checkbox"]:is(:not([checked]), [checked="false"]) > .menu-icon {
