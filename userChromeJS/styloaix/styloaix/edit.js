...
const STYLOAIX_RESOURCE_URI = location.href.replace(/[^/?#]+(?:[?#].*)?$/, '');

function require_mini (m) {
  let scope = {
    exports: {}
  };
  Services.scriptloader.loadSubScript(STYLOAIX_RESOURCE_URI + m + '.js', scope);
...
function initEditor () {
  ...
  sourceEditor.setupAutoCompletion = function () {
    this.extend(require_mini('autocomplete'));
