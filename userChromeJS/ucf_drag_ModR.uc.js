    ...
    link: {
        U: {
            name: "打开链接（新标签，前台）", cmd(val) {
                win.openUILinkIn(val, "tab", this.opts);
            }
        },
        R: {
            name: "打开链接（新标签，后台）", cmd(val) {
                win.openUILinkIn(val, "tabshifted", this.opts);
            }
        },
        RD: {
            name: "另存链接", cmd(val) {
        ...
        D: {
            name: "打开链接（当前标签）", cmd(val) {
                win.openUILinkIn(val, "current", this.opts);
            }
        },
        LD: {
    ...
    text: {
        ...
        "LD-Shift": {
            name: "剑桥词典（新标签，后台）", cmd(val) {
                var TERM = "https://dictionary.cambridge.org/zhs/%E8%AF%8D%E5%85%B8/%E8%8B%B1%E8%AF%AD/" + val;
                if (val)
                    win.openUILinkIn(TERM, "tabshifted", this.opts);
            }
        }
    },
    image: {
