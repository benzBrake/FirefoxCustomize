...
3. **移动脚本到归档目录**：
   ```bash
   mv ScriptName.uc.js 100/ScriptName.uc.js
   ```
4. **添加归档记录**：在归档脚本的 `@note` 中记录
   ```
   // @note 2026-03-29 归档：不再更新，归档至版本目录
   ```
5. **删除主目录原文件**（移动操作已自动完成）

**结果：** 脚本仅存在于归档目录中，主目录不再保留

#### 2. 升级归档
**适用场景：** 需要继续维护并升级到新版本的脚本
