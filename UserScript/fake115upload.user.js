...
// @require      https://cdn.jsdelivr.net/gh/benzBrake/FirefoxCustomize@master/UserScript/libs/waitForKeyElements.js
...
-----END RSA PRIVATE KEY-----'
    ...
    function AddLinkButton(jNode) {
        let topEl = jNode[0].querySelector('.left-tvf');
        let transferBtn = topEl.querySelector('#js_task_pupup_btn');
        let link = document.createElement('a');
        link.setAttribute('href', 'javascript:;');
        link.setAttribute('class', 'button btn-line btn-upload');
        link.setAttribute('menu', 'offline_task');
        link.innerHTML = `<i class="icon-operate ifo-linktask"></i><span>链接任务</span>`;
        topEl.insertBefore(link, transferBtn);
    }
})();